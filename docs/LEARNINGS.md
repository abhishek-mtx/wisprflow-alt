...
Do not assign `sharingType`. On macOS 15 and later `.readWrite` is deprecated, and the AppKit setter drops titled chrome to none. SwiftUI's default is `.readOnly`. Leave it. A SwiftUI `Window` scene is not enough for out\-of\-process Accessibility. Until this process owns a real `NSWindow`, `kAXWindowsAttribute` is the application itself and System Events sees zero windows. Host Studio in `NSWindow` \+ `NSHostingView`. Return false from `applicationShouldTerminateAfterLastWindowClosed`. Do not call `CGSSetWindowSharingState`. Do not call `NSApp.setAccessibilityWindows`, `setAccessibilityElement(true)`, or `setAccessibilityParent(NSApp)` on titled chrome. Reveal with `orderFrontRegardless`, not `makeKeyAndOrderFront`.
