...
Do not assign `sharingType`. On macOS 15 and later `.readWrite` is deprecated, and the AppKit setter drops titled chrome to none. SwiftUI's default is `.readOnly`. Leave it. Post `NSAccessibility.Notification.windowCreated` after Studio is on screen so a fresh launch publishes `AXWindow` instead of `AXApplication`. Do not call `CGSSetWindowSharingState`. Do not call `NSApp.setAccessibilityWindows`, `setAccessibilityElement(true)`, or `setAccessibilityParent(NSApp)` on titled chrome. Reveal with `orderFrontRegardless`, not `makeKeyAndOrderFront`.
